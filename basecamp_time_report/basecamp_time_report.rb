class BasecampTimeReport < Scout::Plugin
  ...
  domain:
    name: Basecamp URI
    notes: foo.basecamphq.com
  api_key:
    name: API Key
  ssl:
    name: Use SSL
    notes: 1 for yes (default), 0 for no
    default: 1
  days_back:
    name: Number of Days
    notes: Number of days back to track time. Defaults to 1
    default: 1
  EOS

  def build_report
    Basecamp.establish_connection!(option(:domain), option(:api_key), 'X', (option(:ssl).to_i == 1))
    report = Basecamp::TimeEntry.report(:from => (Time.now - option(:days_back).to_i.day).strftime('%Y%m%d'), :to => Time.now.strftime('%Y%m%d'))
    people = {}
    total_hours = 0
